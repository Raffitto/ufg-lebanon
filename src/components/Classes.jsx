import { Calendar, Clock, Dumbbell, Flame, HeartPulse, Zap } from 'lucide-react'
...
function ScheduleCard({ item, reduced, index }) {
    ...
    >
      ...
      <div className="relative flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div className="flex min-w-0 items-start gap-4">
          <div className="inline-flex shrink-0 rounded-xl border border-[var(--color-yellow)]/25 bg-[var(--color-yellow)]/10 p-2.5 text-[var(--color-yellow)]">
...
export default function Classes() {
  ...
  const { classes, sections, copy } = activeConfig

  return (
    <SectionReveal id="classes" className="section-padding bg-[var(--color-surface)]/40">
      <div className="mx-auto w-full min-w-0 max-w-7xl">
        ...
        <div className="flex flex-col gap-6 lg:grid lg:grid-cols-2 lg:items-start lg:gap-10">
          <div className="overflow-hidden rounded-2xl border border-white/10 sm:rounded-[var(--radius-xl)]">
            <img
              src={launchMedia.classes}
              alt="Ultimate Fitness Gym group classes at Mtayleb"
              className="aspect-[4/5] w-full object-cover object-center sm:aspect-[3/4] lg:sticky lg:top-24 lg:aspect-auto lg:min-h-[28rem]"
              loading="lazy"
              decoding="async"
            />
          </div>
          ...
          <div className="flex min-w-0 flex-col gap-3 sm:gap-4">
            {classes.items.map((item, index) => (
              <ScheduleCard key={item.id} item={item} reduced={reduced} index={index} />
            ))}
            <div className="pt-2">
              <Button
                variant="secondary"
